...
/** Validates MIME type and decodes Base64 image payloads (optional data URLs). */
public final class ProfileImageUtil {
	...
	public static void validateMime(String mimeType) {
		if (mimeType == null || mimeType.isBlank()) {
			throw new ServiceException(ApiMessages.INVALID_PROFILE_IMAGE, HttpStatus.BAD_REQUEST);
		}
	...
	public static byte[] decodeBase64Image(String base64Payload) {
		...
		try {
			...
			if (raw.length > MAX_BYTES) {
				throw new ServiceException(ApiMessages.PROFILE_IMAGE_TOO_LARGE, HttpStatus.BAD_REQUEST);
			}
			return raw;
